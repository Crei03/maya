...
const totalTasks = computed(() => Number(props.stats?.total || 0));
...
const cards = computed(() => {
    ...
    const inProgressCount = Number(summary.in_progress ?? (byStatus.EN_PROCESO || 0));
    const completedCount = Number(summary.completed ?? (byStatus.COMPLETADA || 0));
    const cancelledCount = Number(summary.cancelled ?? (byStatus.CANCELADA || 0));
    ...
    return [
        ...
        },
        {
            ...
            ringClass: 'ring-2 ring-blue-500 border-transparent',
            activeBarBg: 'bg-blue-500',
        },
        {
            key: 'COMPLETADA',
            label: 'Completados',
